...
namespace UActions.Editor
{
    ...
    public class WorkflowActionRunner
    {
        ...
        public ILogger Logger { set; private get; } = new Logger();
...
//                 }
                ...
                if (Activator.CreateInstance(type) is IAction instance)
                {
                    ...
                    else if (context.Env.Get("ALLOW_INVALID_TARGET", string.Empty) == "true")
                    {
                        instance.Execute(context);
                    }
                    else
